...
/*   Updated: 2024/08/21 21:49:58 by yoshin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	find_solution(unsigned int **map, unsigned int size)
{
	...
	return (recursive(map, size, 0));
}

int	recursive(unsigned int **map, unsigned int size, unsigned int f_count)
{
	unsigned int	max;
	unsigned int	num;
	unsigned int	row;
	...
	col = f_count % size + 1;
	if (map[row][col] != 0)
		return (recursive(map, size, f_count + 1));
	max = size;
	...
	while (++num <= max)
	{
		...
		if (!is_valid_row(map, size, row) || !is_valid_col(map, size, col))
			continue ;
		if (recursive(map, size, f_count + 1))
			return (1);
	}
	map[row][col] = 0;
	return (0);
